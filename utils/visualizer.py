...
class Visualizer:
    ...
    def plot_backtest(self, symbol: str, results: pd.DataFrame):
        ...
        # --- 图1: 价格与信号 ---
        ax1.plot(results.index, results['Close'], label='Close Price', color='black', alpha=0.3)
        ...
        ax1.scatter(buys.index, buys['Close'], marker='^', color='red', s=100, label='Buy Signal', zorder=5)

        # 标注卖出点 (Signal == -1)
        sells = results[results['Signal'] == -1]
        ax1.scatter(sells.index, sells['Close'], marker='v', color='green', s=100, label='Sell Signal', zorder=5)
        ...
        # --- 图2: 资金曲线 ---
        ax2.plot(results.index, results['Equity_Curve'], label='Strategy Equity', color='blue')

        # 填充回撤区域 (如果你在回测引擎里算好了 Drawdown)
        if 'Peak' in results.columns:
            ax2.fill_between(results.index, results['Equity_Curve'], results['Peak'],
        ...
        # 调整布局并保存
        plt.tight_layout()
        save_path = os.path.join(self.save_dir, f"{symbol}_report.png")
